...
GAME_RANDOM_OFFSET_OBSTACLE = random.random() * 0.9 * GAME_RANDOM_OFFSET_GATE

GAME_WORLDS =[

    # World 1 - Actual Gate Task
    (("Enter the Gate!", "YAY! We passed through the gate!"),[
        {"type" : None, "goal_id": 1, "rel" : None, "colour" : None,        "world" : (-0.1, 9.85) },
        {"type" : None, "goal_id": 1, "rel" : None, "colour" : None,        "world" : (0.1, 9.85) },
        {"type" : None, "goal_id": 2, "rel" : None, "colour" : None,        "world" : (-0.1, 10.2) },
        {"type" : None, "goal_id": 2, "rel" : None, "colour" : None,        "world" : (0.1, 10.2) },
    ...
    # World 2 - Qualifiers
    (("Enter the Gate!", "YAY! We passed qualifiers!"),[
        {"type" : None, "goal_id": 1, "rel" : None, "colour" : None,        "world" : (-0.1, 9.85) },
        {"type" : None, "goal_id": 1, "rel" : None, "colour" : None,        "world" : (0.1, 9.85) },
        {"type" : None, "goal_id": 2, "rel" : None, "colour" : None,        "world" : (-0.1, 10.15) },
        {"type" : None, "goal_id": 2, "rel" : None, "colour" : None,        "world" : (0.1, 10.15) },
    ...
    (("Enter the Gate!", "YAY! We passed through the gate!"),[
        ...
        {"type" : None, "goal_id": 1, "rel" : None, "colour" : None,        "world" : (GAME_RANDOM_OFFSET_GATE+0.1, 9.85) },
        {"type" : None, "goal_id": 2, "rel" : None, "colour" : None,        "world" : (GAME_RANDOM_OFFSET_GATE-0.1, 10.15) },
        {"type" : None, "goal_id": 2, "rel" : None, "colour" : None,        "world" : (GAME_RANDOM_OFFSET_GATE+0.1, 10.15) },
...
WINDOW_HEIGHT = 500 # in pixels
WINDOW_RATIO_AERIAL_FRONT_VIEW = 0.6

AERIAL_RADIUS = 7          # in metres
CAMERA_FOV_RADS = np.deg2rad(90) # in rads
AERIAL_FOV_RADIUS = 5      # in metres

PLAYER_DEPTH_STEP_ENU = 0.1 # in metres
PLAYER_FWD_STEP_ENU   = 0.1 # in metres
PLAYER_SIDE_STEP_ENU  = 0.1 # in metres
PLAYER_YAW_STEP_ENU   = 1   # in degs

GAME_DEPTH_POOL_MAX   = 1.8 # in metres
GAME_DEPTH_POOL_CRASH = 0.8 # in metres
GAME_DEPTH_POOL_LIMIT = 1.6 # in metres

GAME_ZONE_START_LEN   = 0.5 # in metres
GAME_ZONE_LOWER_X     = -15.0 # in metres
GAME_ZONE_LOWER_Y     = -1.00 # in metres
GAME_ZONE_UPPER_X     = 15.0 # in metres
GAME_ZONE_UPPER_Y     = 20.0 # in metres

GAME_TILE_SCALE = 6
GAME_AREA_TILE_HEIGHT = GAME_TILE_SCALE * 0.240 # in metres
GAME_AREA_TILE_WIDTH  = GAME_TILE_SCALE * 0.115 # in metres

GAME_CURRENT_WORLD = 0

GAME_CONTROL_DEPTH_RESPONSE = 0.030 # per iteration
GAME_CONTROL_FWD_RESPONSE   = 0.020 # per iteration
GAME_CONTROL_SIDE_RESPONSE  = 0.020 # per iteration
GAME_CONTROL_YAW_RESPONSE   = 0.025 # per iteration

GAME_ENVIRONMENT_BUOYANCY   = (random.random()/2+0.5) * 0.001
GAME_ENVIRONMENT_DRIFT_X    = randomSign()*(random.random()/2+0.5) * 0.0001
GAME_ENVIRONMENT_DRIFT_Y    = randomSign()*(random.random()/2+0.5) * 0.0001
...
def draw_vectical_gradient_rect(window, top_colour, bottom_colour, target_rect):
    ...
    colour_rect = pygame.transform.smoothscale(colour_rect, (target_rect.width, target_rect.height+1))  # stretch!
...
class SimpleAUVSimulatorGame:
    def __init__(self, depthPIDController, isPerfectControl=False):
        self.aerial_radius = AERIAL_RADIUS

        self.restart()

        self.grid = list()
        self.grid_play_zone  = list()
        self.grid_start_zone = list()
        ...
        aerial_window_w = int(WINDOW_RATIO_AERIAL_FRONT_VIEW * WINDOW_WIDTH)
        sub_window_h = (WINDOW_HEIGHT // 2) - 5 
        sub_window_w = WINDOW_WIDTH - aerial_window_w - 10

        # Create a game window
        self.view_main_window = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))
        self.view_aerial  = pygame.Surface((aerial_window_w, WINDOW_HEIGHT))
        ...
        self.font_large = pygame.font.Font(pygame.font.get_default_font(), 29)
    ...
    def restart(self):
        ...
        self.global_y = 0
        self.global_yaw = 0
        self.global_altitude = 0 #GAME_DEPTH_POOL_MAX

        self.plot_path = [(0,0), (0,0)]

        self.game_phrases = GAME_WORLDS[GAME_CURRENT_WORLD][0]
        ...
        assert (not 0 in type_id_max), f"goal_ids must be larger than 0 {type_id_max}"

        self.goals_achieved = [False] * (max(type_id_max)+1)
        self.goals_achieved[0] = True
    ...
    def updateObjectPositions(self, auv_tf : np.array):
        ...
        self.grid = (rot_tf @ np.array(grid_temp).T).T
        
        # Update start Zone Grid
        grid_play_zone = [[GAME_ZONE_LOWER_X, GAME_ZONE_LOWER_X, GAME_ZONE_UPPER_X, GAME_ZONE_UPPER_X],
                          [GAME_ZONE_LOWER_Y, GAME_ZONE_UPPER_Y, GAME_ZONE_UPPER_Y, GAME_ZONE_LOWER_Y],
                           [1, 1, 1, 1]]
        self.grid_play_zone = (auv_tf @ np.array(grid_play_zone)).T

        # Update start Zone Grid
        grid_start_zone = [[GAME_ZONE_START_LEN, GAME_ZONE_START_LEN, -GAME_ZONE_START_LEN, -GAME_ZONE_START_LEN],
                           [GAME_ZONE_START_LEN, -GAME_ZONE_START_LEN, -GAME_ZONE_START_LEN, GAME_ZONE_START_LEN],
                           [1, 1, 1, 1]]
        self.grid_start_zone = (auv_tf @ np.array(grid_start_zone)).T
    ...
    def updateMovement(self, rel_fwd_enu : float, rel_side_enu : float, rel_turn_enu : float):
        ...
        #    rel_depth_enu = np.clip(self.depth_PID_controller(self.global_altitude, tick_diff), -1.0, 1.0) * PLAYER_DEPTH_STEP_ENU
        #    self.depth_controller_last_tick = time.time()

        # Adds noise to the controls
        if not self.isPerfectControl:
            self.vec_depth = GAME_CONTROL_DEPTH_RESPONSE*rel_depth_enu + (1-GAME_CONTROL_DEPTH_RESPONSE)*self.vec_depth - GAME_ENVIRONMENT_BUOYANCY
            self.vec_fwd   = GAME_CONTROL_FWD_RESPONSE*rel_fwd_enu     + (1-GAME_CONTROL_FWD_RESPONSE)*self.vec_fwd     + GAME_ENVIRONMENT_DRIFT_X
            self.vec_side  = GAME_CONTROL_SIDE_RESPONSE*rel_side_enu   + (1-GAME_CONTROL_SIDE_RESPONSE)*self.vec_side   + GAME_ENVIRONMENT_DRIFT_Y
            self.vec_yaw   = GAME_CONTROL_YAW_RESPONSE*rel_yaw_enu     + (1-GAME_CONTROL_YAW_RESPONSE)*self.vec_yaw
            
            rel_depth_enu = self.vec_depth
            rel_fwd_enu   = self.vec_fwd
            rel_side_enu  = self.vec_side
            rel_yaw_enu   = self.vec_yaw

            # Update Depth
            #self.global_altitude -= rel_depth_enu
            #self.global_altitude = max(min(self.global_altitude, GAME_DEPTH_POOL_MAX), 0)
            
        else:
            #self.global_altitude = GAME_DEPTH_POOL_LIMIT - 0.3
            pass
        ...
        # Return world in baselink transform frame
    ...
    def checkEndCondition(self):
        ...
        # Check for the auv is within play zone
        if not (GAME_ZONE_LOWER_X <= self.global_x <= GAME_ZONE_UPPER_X and GAME_ZONE_LOWER_Y <= self.global_y <= GAME_ZONE_UPPER_Y):
            self.displayResult("Don't leave the play field!", (255, 75, 56))
            return
        ...
        if self.time_diff > 60*10:
            self.displayResult("Time's up! Mission Failed", (255, 75, 56))
            
            
        # Check if initial dive goal was achieved
        #if self.global_altitude < GAME_DEPTH_POOL_LIMIT:
        #    self.goals_achieved[0] = True
    ...
    def updateAerialView(self, auv_tf : np.array) -> None:
        ...
        # Draw Window
        pygame.draw.rect(self.view_aerial, (173, 216, 230), self.view_aerial.get_rect(), 0, border_bottom_left_radius=20)
       
        # Draw Grid
        grid_temp = scale_factor * self.grid
        for px1, py1, _, px2, py2, _ in grid_temp:
            pygame.draw.line(self.view_aerial, (0,0,0), (win_cx+px1, win_cy-py1), (win_cx+px2, win_cy-py2), 1)

        # Draw Start Zone
        grid_temp_start_zone = scale_factor * self.grid_start_zone
        len_grid_start_zone = len(grid_temp_start_zone)
        for pt_idx in range(len_grid_start_zone):
            px1, py1, _ = grid_temp_start_zone[pt_idx]
            px2, py2, _ = grid_temp_start_zone[(pt_idx+1)%len_grid_start_zone]
            pygame.draw.line(self.view_aerial, (155, 89, 182), (win_cx+px1, win_cy-py1), (win_cx+px2, win_cy-py2), 3)
        
            del px1, py1, px2, py2

        # Draw Play Zone
        grid_temp_play_zone = scale_factor * self.grid_play_zone
        len_grid_play_zone = len(grid_temp_play_zone)
        for pt_idx in range(len_grid_play_zone):
            px1, py1, _ = grid_temp_play_zone[pt_idx]
            px2, py2, _ = grid_temp_play_zone[(pt_idx+1)%len_grid_play_zone]
            pygame.draw.line(self.view_aerial, (192, 57, 43), (win_cx+px1, win_cy-py1), (win_cx+px2, win_cy-py2), 3)
        ...
        self.view_aerial.blit(text_surface, (compass_offset[0]-(text_surface.get_width() // 2), win_height-20))
        ...
        text_surface = self.font_small.render(f"{reference_scale}m", True, legend_colour, (149, 165, 166))
        self.view_aerial.blit(text_surface, (win_width-30, win_height-25))

        # Draw altitude scale
        #altitude_scale_x_offset = 25
        #altitude_scale_y_offset = 30
        #altitude_scale_h = win_height-altitude_scale_y_offset-60
        #altitude_scale_limit_h  = (1-(GAME_DEPTH_POOL_LIMIT / GAME_DEPTH_POOL_MAX))*altitude_scale_h + altitude_scale_y_offset
        #altitude_scale_crash_h  = (1-(GAME_DEPTH_POOL_CRASH / GAME_DEPTH_POOL_MAX))*altitude_scale_h + altitude_scale_y_offset
        #altitude_scale_danger_limit_h = altitude_scale_limit_h+(0.2 / GAME_DEPTH_POOL_MAX)*altitude_scale_h
        #altitude_scale_danger_crash_h = altitude_scale_crash_h-(0.2 / GAME_DEPTH_POOL_MAX)*altitude_scale_h
        #
        #pygame.draw.line(self.view_aerial, (241, 196, 15), (altitude_scale_x_offset, altitude_scale_y_offset),      (altitude_scale_x_offset, altitude_scale_limit_h), 10)
        #pygame.draw.line(self.view_aerial, (39, 160, 50),  (altitude_scale_x_offset, altitude_scale_limit_h),(altitude_scale_x_offset, altitude_scale_danger_crash_h), 10)
        #pygame.draw.line(self.view_aerial, (241, 196, 15), (altitude_scale_x_offset, altitude_scale_danger_crash_h),(altitude_scale_x_offset, altitude_scale_crash_h), 10)
        #pygame.draw.line(self.view_aerial, (192, 57, 43),  (altitude_scale_x_offset, altitude_scale_crash_h),       (altitude_scale_x_offset, altitude_scale_h), 10)

        # Draw altitude information
        #altitude_pos_y = (1-(self.global_altitude / GAME_DEPTH_POOL_MAX))*(altitude_scale_h-altitude_scale_y_offset) + altitude_scale_y_offset 
        #altitude_text_colour = (241, 196, 15) # Good altitude
        #if altitude_pos_y > altitude_scale_crash_h:
        #    altitude_text_colour = (192, 57, 43) # Bad altitude
        #elif altitude_scale_limit_h <= altitude_pos_y <= altitude_scale_danger_crash_h:
        #    altitude_text_colour = (39, 160, 50) # Danger altitude
        #
        #pygame.draw.polygon(self.view_aerial, altitude_text_colour, [   \
        #    (altitude_scale_x_offset-20, altitude_pos_y-8), \
        #    (altitude_scale_x_offset-5, altitude_pos_y),    \
        #    (altitude_scale_x_offset-20, altitude_pos_y+8)])
        #
        #text_surface = self.font_small.render("Alt: {:.2f}m".format(self.global_altitude), True, altitude_text_colour, (149, 165, 166))
        #self.view_aerial.blit(text_surface, (altitude_scale_x_offset-15, altitude_scale_h+8))

        # Draw Objective text
        text_surface = self.font_small.render(self.view_display_objective, True, legend_colour, (149, 165, 166))
        self.view_aerial.blit(text_surface, (win_width-text_surface.get_width()-5, 10))
    ...
    def updateFrontView(self) -> None:
        ...
        if self.view_frontal_base is None:
            ...
            # Draw Window
            pygame.draw.rect(self.view_frontal_base, (255,255,255), self.view_frontal.get_rect(), 0)
    ...
    def updatePlotView(self) -> None:
        ...
        # Draw Window
        pygame.draw.rect(self.view_plot, (255,165,0), self.view_plot.get_rect(), 0, border_top_left_radius=10, border_bottom_right_radius=20)
    ...
    def updateView(self, auv_tf : np.array):
        ...
        aerial_window_w = int(WINDOW_RATIO_AERIAL_FRONT_VIEW * WINDOW_WIDTH)+10
        sub_window_h = (WINDOW_HEIGHT // 2) + 6

        self.view_main_window.blit(self.view_aerial,  pygame.Rect(0, 0, self.view_aerial.get_width(), self.view_aerial.get_height()))
        self.view_main_window.blit(self.view_frontal, pygame.Rect(aerial_window_w, 0, self.view_frontal.get_width(), self.view_frontal.get_height()))
        self.view_main_window.blit(self.view_plot,    pygame.Rect(aerial_window_w, sub_window_h, self.view_plot.get_width(), self.view_plot.get_height()))
